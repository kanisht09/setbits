...
LL setbits(LL n){
	LL cnt=0;
	while(n>0){
		cnt+= (n&1);	//checking if the last bit is set or not
		n=n>>1;		//right shifting bits of n
	}
	return cnt;	//returning the total no of setbits in n
}
void solve(LL tc)
{
	LL n;
	cin>>n;
	cout<<setbits(n)<<endl;	//Another apprpach for counting set bits
	cout<<__builtin_popcount(n)<<endl; 	//Using in-built function for counting set bits
}
int main()
{
        fastIO
